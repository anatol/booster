 ...
 * `booster.debug` enable booster debug output. It is printed to console at the boot time. This feature might be useful to debug booster issues.
   The debug log is also printed to kernel kmsg buffer and available for reading either with `dmesg` or with `journalctl -b`. If booster.debug is enabled then kmsg throttling gets disabled automatically.

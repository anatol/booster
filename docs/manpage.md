 ...
 * `rd.luks.header=$UUID=$PATH` detached LUKS header for the device identified by `$UUID` — the LUKS superblock UUID, the same value `rd.luks.uuid` takes and what `cryptsetup luksUUID $PATH` prints. Because that UUID lives *in the header*, a device with a detached header exposes no LUKS or filesystem UUID of its own; identify it by this header UUID, not by the data partition's `PARTUUID`. `$PATH` can take three forms:
 ...
 * `rd.luks.data=$UUID=$deviceref` pins the data device for the LUKS volume identified by `$UUID` (its LUKS superblock UUID). `$deviceref` is `PARTUUID=...`, `PARTLABEL=...`, `LABEL=...`, `UUID=...`, or a path. Required to unlock more than one device with a detached header from the kernel command line: a data device with a detached header carries no identifier of its own, and `rd.luks.header=` alone supplies only the header.

...
func initPlymouth() {
	...
	// Wait for a usable DRM device. Prefer a real GPU over simpledrm since
	// simpledrm's card may be removed when the real GPU driver takes over.
	if !waitForDrmDevice(5 * time.Second) {
		warning("plymouth: no DRM device found, falling back to no splash")
		plymouthEnabled = false
		return
	}
...
func isSimpledrm(devPath string) bool {
	...
	return filepath.Base(driverLink) == "simpledrm"
}
...
// every DRM device, falling back to text-only mode after an 8s timeout.
//
